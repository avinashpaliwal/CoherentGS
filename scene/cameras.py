...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        self.original_image = image.clamp(0.0, 1.0).to(self.data_device).detach()
        self.factor = 1
        self.image_width = self.factor*self.original_image.shape[2]
        self.image_height = self.factor*self.original_image.shape[1]

        if gt_alpha_mask is not None:
            self.original_image *= gt_alpha_mask.to(self.data_device)
        else:
            self.original_image *= torch.ones((1, self.image_height//self.factor, self.image_width//self.factor), device=self.data_device)
    ...
    def refactor(self, factor, fov_scale=None):
        ...
        gt_alpha_mask = self.gt_alpha_mask
        if gt_alpha_mask is not None:
            self.original_image *= gt_alpha_mask.to(self.data_device)
        # else:
        #     self.original_image *= torch.ones((1, self.image_height//self.factor, self.image_width//self.factor), device=self.data_device)
